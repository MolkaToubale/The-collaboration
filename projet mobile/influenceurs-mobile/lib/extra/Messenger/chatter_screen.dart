...
import 'package:projet/extra/Messenger/user_image_network_bounce.dart';
...
class _ChatterScreenState extends State<ChatterScreen>
  ...
  //Cette méthode sert à ajouter un emoji sélectionné à la fin du texte du champ de saisie de message et déplacer le curseur à la fin
...
//Fonction permettant la suppression de l'émoji suite à sa selection à partir du catalogue d'émojis
  _onBackspacePressed() {
    chatMsgTextController
 ...
 //Télécharger l'image dans firestore en créant une référence vers l'emplacement de stockage spécifié
 ...
 //Télécharger l'image dans un emplacement spécifique +obtention d'une URL+création d'une conversation avec cette image non nulle
...
//Sélectionner une image depuis la caméra ou la galerie
          ...
          child: AppBar(
            ...
            title: StreamBuilder(
                ...
                builder: (context, AsyncSnapshot<DocumentSnapshot> snapshot) {
                  if (snapshot.connectionState == ConnectionState.active &&  //si la connexion au flux est active ou pas
                      snapshot.data != null) {
      ...
      body: Directionality(
        ...
        child: SafeArea(
          ...
          child: Column(
            ...
            children: [
              //affiche un flux de messages de chat.
              Expanded(
                  child: ChatStream(
              ...
              //Apparition de l'emoji selector
              Offstage(
                offstage: !emojiShowing,
                child: SizedBox(
                  height: 250, //hauteur de l'emoji selector
                  child: EmojiPicker(
                    ...
                    children: [
                      Material(
                        ...
                        shape: RoundedRectangleBorder(
                            ...
                            //Icone pour faire apparaître l'emoji selector ou le faire disparaitre
                        child: IconButton(
                          onPressed: () {
                      ...
                      Material(
                        ...
                        child: Consumer<UserProvider>(
                            ...
                            //Ce bouton permet à l'utilisateur de sélectionner une image à partir de la galerie
                                ? const IconButton(
                                    onPressed: null,
                                    icon: Icon(
                                      Icons.photo,
                      ...
                      Material(
                        ...
                        child: Consumer<UserProvider>(
                          builder: (_, provider, __) => provider.isProcessing
                              ? const MyLoading()
                              //Ce  bouton qui permet à l'utilisateur d'envoyer un message
                              : IconButton(
                                  onPressed: () async {
                                    if (
                                        ...
                                        chatMsgTextController.text.trim() !=
                                            '') {
                                      final resultString =
                                          chatMsgTextController.text.replaceAll(RegExp( r'(?:[\t ]*(?:\r?\n|\r))+'),'\n'); //supprimer l'excés de saut de lignes
  ...
  void initState() {
    ...
    conversationProvider =Provider.of<MessagesProvider>(context, listen: false);
...
//
  ...
  void dispose() async {
    ...
    // WidgetsBinding est une classe qui fournit des informations sur l'état de la liaison des widgets avec les couches inférieures du framework.
    //removeObserver retire l'observateur concerné de la liste des observateurs
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData) {
            return Center(
              ...
              children: [
                GradientText(
                    "Faites Coucou à ${context.read<ChatRoomProvider>().userto!.name}",
          ...
          if (snapshot.connectionState == ConnectionState.active) {
            ...
            return ListView.builder(
                ...
                reverse: true, //improves the ability of the listview to estimate the max scroll extend
                padding:const EdgeInsets.symmetric(vertical: 10, horizontal: 5),
                itemCount: messages!.length,
                itemBuilder: (context, index) {
                  String? lastMsgWithSeenId = messages
                          .where((element) =>
                              element.seenBy!.isNotEmpty && // vérifie si la propriété seenBy de chaque élément de messages n'est pas vide.
                              element.from == userForm!.id)
                          .isNotEmpty //si cond vérifiée
                      ? messages
                          .where((element) =>
                              element.seenBy!.isNotEmpty &&
                              element.from == userForm!.id)
                          .first
                          .id//extraction de l'id du dernier message vu
                      : null;
                      //'lastMsgWithSeenId' contiendra l'ID du dernier message vu par l'utilisateur naviguant au sein du formulaire de messages et s'il n'y a aucun message vu par ce dernier elle sera nulle.
                  ...
                  final icu = Provider.of<UserProvider>(context, listen: false)
                          .currentUser!
                          .id ==
                      userId; //récupération de l'id de l'utilisateur ayant envoyé le message

                  if (index == 0) {
                    previousIcu = true;
                  } else {
                   
                    previousIcu = messages[i].from == messages[index].from; //previousIcu vérifie si l'utilisateur du dernier message est le même qui a envoyé l'avant dernier message ou pas
                  }
                        ...
                        builder: (context, provider, _) => ChatBubble(
                            mTest: lastMsgWithSeenId != null
                                ? lastMsgWithSeenId == messages[index].id
                                    ? true
                                    : false
                                : false, //true si ce message est le dernier message vu par l'utilisateur.
