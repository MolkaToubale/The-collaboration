...
//Cette méthode arrête l'écoute des flux de données
...
//Cette méthode écoute les modifications en temps réel dans une collection de documents Firestore, met à jour les propriétés "seenBy" des documents modifiés 
      ...
      if (event.docChanges.isNotEmpty) {
        ...
        if ((event.docChanges.length != event.docs.length &&
            ...
            (event.docs.length == event.docChanges.length &&
                event.docChanges.length == 1)) //une seule modification
                {
...
//cette méthode parcourt une liste de messages et met à jour les messages non vus par l'utilisateur actuel puis ajoute l'id de l'utilisateur courant dans le document du message concerné
 ...
 //Même concept que _seenByConfiguration la différence se trouve au niveau des paramètres
  ...
  String? _generateUniqueRoomChatId(String? a, String? b) {
    int length = a!.length > b!.length ? b.length : a.length; //sélectionner l'id le plus court
