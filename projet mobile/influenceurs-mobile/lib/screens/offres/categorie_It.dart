...
class CategorieIT extends StatefulWidget {
  const CategorieIT({super.key});

  @override
  CategorieITState createState() => CategorieITState();
}

class CategorieITState extends State<CategorieIT> {
  List<OffreModel> offresIT = [];

  Future<void> fetchOffresIt() async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("IT", style: style.title.copyWith(fontSize: 20)),
          actions: [
  ...
  onTap: () async {
      ...
      // Sinon, reprendre la lecture
          await speak('Cette interface vous permet de visualiser toutes les offers appartenant à la catégorie IT pour lesquelles vous pouvez postuler à savoir  ${offresIT.length.toString()} offres. Vous pouvez également effectuer une recherche à partir de la barre de recherche ou bien retourner à l\'interface d\'Accueil grâce à la flêche située à gauche en hait de la page. ');
          ...
          children: [
            ...
            Expanded(
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  crossAxisSpacing: 10),
              itemBuilder: (_, index) {
                return OffreWidget(offre: offresIT[index]);
              },
            )),
          ],
        ),
