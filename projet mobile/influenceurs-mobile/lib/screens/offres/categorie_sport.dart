...
class CategorieSport extends StatefulWidget {
  const CategorieSport({super.key});

  @override
 CategorieSportState createState() =>CategorieSportState();
}

class CategorieSportState extends State<CategorieSport> {
  List<OffreModel> offresSport = [];

  Future<void> fetchOffresSport() async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Sport", style: style.title.copyWith(fontSize: 20)),
         actions: [
  ...
  onTap: () async {
      ...
      // Sinon, reprendre la lecture
          await speak('Cette interface vous permet de visualiser toutes les offers appartenant à la catégorie Sport pour lesquelles vous pouvez postuler à savoir  ${offresSport.length.toString()} offres. Vous pouvez également effectuer une recherche à partir de la barre de recherche ou bien retourner à l\'interface d\'Accueil grâce à la flêche située à gauche en hait de la page. ');
          ...
          children: [
            ...
            Expanded(
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  crossAxisSpacing: 10),
              itemBuilder: (_, index) {
                return OffreWidget(offre: offresSport[index]);
              },
            )),
          ],
        ),
