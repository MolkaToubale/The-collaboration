...
class CategoriePhotographie extends StatefulWidget {
  const CategoriePhotographie({super.key});

  @override
 CategoriePhotographieState createState() =>CategoriePhotographieState();
}

class CategoriePhotographieState extends State<CategoriePhotographie> {
  List<OffreModel> offresPhotographie = [];

  Future<void> fetchOffresPhotographie() async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Photographie", style: style.title.copyWith(fontSize: 20)),
         actions: [
  ...
  onTap: () async {
      ...
      // Sinon, reprendre la lecture
          await speak('Cette interface vous permet de visualiser toutes les offers appartenant à la catégorie Photographie pour lesquelles vous pouvez postuler à savoir  ${offresPhotographie.length.toString()} offres. Vous pouvez également effectuer une recherche à partir de la barre de recherche ou bien retourner à l\'interface d\'Accueil grâce à la flêche située à gauche en hait de la page. ');
        ...
        child: Column(
          children: [
            ...
            Expanded(
              ...
              itemCount: offresPhotographie.length,
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  crossAxisSpacing: 10),
              itemBuilder: (_, index) {
                return OffreWidget(offre: offresPhotographie[index]);
              },
            )),
          ],
        ),
