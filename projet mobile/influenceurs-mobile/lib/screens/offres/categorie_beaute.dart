...
class CategorieBeaute extends StatefulWidget {
  const CategorieBeaute({super.key});

  @override
  CategorieBeauteState createState() => CategorieBeauteState();
}

class CategorieBeauteState extends State<CategorieBeaute> {
  List<OffreModel> offresBeaute = [];

  Future<void> fetchOffresBeaute() async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Beauté et Bien-être", style: style.title.copyWith(fontSize: 20)),
          actions: [
  ...
  onTap: () async {
      ...
      // Sinon, reprendre la lecture
          await speak('Cette interface vous permet de visualiser toutes les offers appartenant à la catégorie Beauté et Bien-être pour lesquelles vous pouvez postuler à savoir  ${offresBeaute.length.toString()} offres. Vous pouvez également effectuer une recherche à partir de la barre de recherche ou bien retourner à l\'interface d\'Accueil grâce à la flêche située à gauche en hait de la page. ');
          ...
          children: [
            ...
            Expanded(
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  crossAxisSpacing: 10),
              itemBuilder: (_, index) {
                return OffreWidget(offre: offresBeaute[index]);
              },
            )),
          ],
        ),
